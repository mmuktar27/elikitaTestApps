...
import {
  ...
  Users,
  UserCheck,
  X,CalendarCheck,FlaskConical,ClipboardList,Boxes
} from "lucide-react";
 ...
 const { activeAppointmentPage, setActiveAppointmentPage } = useAppointmentPage();
  ...
  return (
    <Link
      ...
      onClick={(e) => {
        if (url === "/labtechnician/patients") setActivePatientsPage("patient");
        if (url === "/labtechnician/referrals") setActiveReferralsPage("referral");
        if (url === "/labtechnician/appointments") setActiveAppointmentPage("appointment");
...
function LayoutPage({ children }) {
  ...
  <NavItem
    icon={Home}
    label="Dashboard"
    active={routes === "/labtechnician"}
    url={"/labtechnician"}
    onClick={() => isSidebarOpen && toggleSidebar()} // Close sidebar on mobile
  />
  <NavItem
  icon={FlaskConical} // Icon representing lab tests
  label="Test Request"
  active={routes.includes("/labtechnician/testrequest")}
  url="/labtechnician/testrequest"
  onClick={() => {
    setActivePatientsPage("testrequest");
    if (isSidebarOpen) toggleSidebar();
  }}
/>

<NavItem
  icon={ClipboardList} // Indicates reviewing or managing requests
  label="Review Requests"
  active={routes.includes("/labtechnician/escalatedtest")}
  url="/labtechnician/escalatedtest"
  onClick={() => {
    setActiveReferralsPage("escalatedtest");
    if (isSidebarOpen) toggleSidebar();
  }}
/>

<NavItem
  icon={Boxes} // Icon representing inventory stock
  label="Inventory"
  active={routes === "/labtechnician/inventory"}
  url="/labtechnician/inventory"
  onClick={() => {
    setActiveAppointmentPage("inventory");
    if (isSidebarOpen) toggleSidebar();
  }}
/>
  ...
  <NavItem
    icon={UserCheck}
    label="Appointments"
    active={routes === "/labtechnician/appointments"}
    url={"/labtechnician/appointments"}
    onClick={() => {
  ...
  <NavItem
    icon={Activity}
    label="Events"
    active={routes === "/labtechnician/events"}
    url={"/labtechnician/events"}
  ...
  <NavItem
    icon={UserRoundPen}
    label="Profile"
    active={routes === "/labtechnician/profile"}
    url={"/labtechnician/profile"}
    onClick={() => isSidebarOpen && toggleSidebar()}
  />
  <button
  ...
  <div className="fixed inset-x-0 top-0 z-40 h-auto min-h-20 bg-gray-100 p-8 md:left-64">
    <div className="flex h-full items-center justify-between"> 
      <div className="flex items-center">
        ...
        <h1 className="text-2xl font-bold text-[#007664] md:text-3xl">Labtechnicians</h1>
      </div>
      <div className="flex items-center">
        <div className="flex cursor-pointer items-center">
          <Link href="/labtechnician/profile" className="flex items-center gap-2">
            <Avatar
