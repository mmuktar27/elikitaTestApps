...
import {
  ...
  Users,
  UserCheck,
  X,CalendarCheck,FileText,History	,RotateCcw,ShieldCheck,Package
} from "lucide-react";
 ...
 const { activeAppointmentPage, setActiveAppointmentPage } = useAppointmentPage();
  ...
  return (
    <Link
      ...
      onClick={(e) => {
        if (url === "/cashier/patients") setActivePatientsPage("patient");
        if (url === "/cashier/referrals") setActiveReferralsPage("referral");
        if (url === "/cashier/appointments") setActiveAppointmentPage("appointment");
...
function LayoutPage({ children }) {
  ...
  <NavItem
    icon={Home}
    label="Dashboard"
    active={routes === "/cashier"}
    url={"/cashier"}
    onClick={() => isSidebarOpen && toggleSidebar()} // Close sidebar on mobile
  />
  <NavItem
  icon={FileText} // Better suited for billing documents
  label="Billings"
  active={routes.includes("/cashier/billings")}
  url="/cashier/billings"
  onClick={() => {
    setActivePatientsPage("billings");
    if (isSidebarOpen) toggleSidebar();
  }}
/>

<NavItem
  icon={History} // Better match for transaction history
  label="Transactions History"
  active={routes.includes("/cashier/transactionshistory")}
  url="/cashier/transactionshistory"
  onClick={() => {
    setActiveReferralsPage("transactionshistory");
    if (isSidebarOpen) toggleSidebar();
  }}
/>

<NavItem
  icon={RotateCcw} // Common icon for refunds and adjustments
  label="Refunds & Adjustments"
  active={routes === "/cashier/refunds"}
  url="/cashier/refunds"
  onClick={() => {
    setActiveAppointmentPage("refunds");
    if (isSidebarOpen) toggleSidebar();
  }}
/>

<NavItem
  icon={ShieldCheck} // Symbolic of insurance protection/claims
  label="Insurance Claims"
  active={routes === "/cashier/insuranceclaims"}
  url="/cashier/insuranceclaims"
  onClick={() => isSidebarOpen && toggleSidebar()}
/>

<NavItem
  icon={Package} // Represents services or packages offered
  label="Services"
  active={routes === "/cashier/services"}
  url="/cashier/services"
  onClick={() => isSidebarOpen && toggleSidebar()}
/>
  ...
  <div className="fixed inset-x-0 top-0 z-40 h-auto min-h-20 bg-gray-100 p-8 md:left-64">
    <div className="flex h-full items-center justify-between"> 
      <div className="flex items-center">
        ...
        <h1 className="text-2xl font-bold text-[#007664] md:text-3xl">Cashiers</h1>
      </div>
      <div className="flex items-center">
