 ...
 const { activeAppointmentPage, setActiveAppointmentPage } = useAppointmentPage();
  ...
  return (
    <Link
      ...
      onClick={(e) => {
        if (url === "/pharmacy/patients") setActivePatientsPage("patient");
        if (url === "/pharmacy/referrals") setActiveReferralsPage("referral");
        if (url === "/pharmacy/appointments") setActiveAppointmentPage("appointment");
...
function LayoutPage({ children }) {
  ...
  <NavItem
    icon={Home}
    label="Dashboard"
    active={routes === "/pharmacy"}
    url={"/pharmacy"}
    onClick={() => isSidebarOpen && toggleSidebar()} // Close sidebar on mobile
  />
  <NavItem
  icon={Pill} // Best suited for drug/medication-related actions
  label="Drugs Request"
  active={routes.includes("/pharmacy/drugsrequest")}
  url="/pharmacy/drugsrequest"
  onClick={() => {
...
<NavItem
  icon={Boxes} // Represents storage, stock, or inventory
  label="Inventory"
  active={routes.includes("/pharmacy/inventory")}
  url="/pharmacy/inventory"
  onClick={() => {
    setActiveReferralsPage("inventory");
    if (isSidebarOpen) toggleSidebar();
  }}
/>
  ...
  <NavItem
    icon={UserCheck}
    label="Appointments"
    active={routes === "/pharmacy/appointments"}
  ...
  <NavItem
    icon={UserRoundPen}
    label="Profile"
    active={routes === "/pharmacy/profile"}
  ...
  <div className="fixed inset-x-0 top-0 z-40 h-auto min-h-20 bg-gray-100 p-8 md:left-64">
    <div className="flex h-full items-center justify-between"> 
      <div className="flex items-center">
        ...
        <h1 className="text-2xl font-bold text-[#007664] md:text-3xl">Pharmacy</h1>
      </div>
      <div className="flex items-center">
